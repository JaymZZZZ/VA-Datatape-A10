	...
	Public Sub Main()
		...
		If dir_found = false then
			If file_code <> nothing then
				VA.WriteToLog("Advanced File Format: a10_datatape_"+file_code+"_<ANYTHING_YOU_WANT_HERE>.json", "red")
			Else
				VA.WriteToLog("Advanced File Format: a10_datatape_alpha_<ANYTHING_YOU_WANT_HERE>.json", "red")
			End If
			VA.WriteToLog("Basic File Format: a10_datatape.json", "red")
		...
		If file_code = "" then
			If File.exists(datatape_dir + "\a10_datatape.json") then
				datatape_file = datatape_dir + "\a10_datatape.json"
				file_found = true
				json_file_found = true
			End If
		Else
			datatape_file = Dir(datatape_dir + "\a10_datatape_" + file_code + "*.json")
		...
		If file_found = false then
			...
			Else
				VA.WriteToLog("Advanced File Format: a10_datatape_alpha_<ANYTHING_YOU_WANT_HERE>.json", "red")
			End If
			VA.WriteToLog("Basic File Format: datatape.json", "red")
			VA.WriteToLog("File name must match one of the following formats:", "red")
		...
		For Each item As Dictionary(Of String, Object) In data("drawings")
			...
			If item("type") = "circle" then
				If stpt > 1 then 
					...
					Dim name_string as string = item("name")
					...
					wpt_name = wpt_name.replace("/", "_").replace(" ", "_")
